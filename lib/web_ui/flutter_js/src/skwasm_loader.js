...
export const loadSkwasm = (deps, config, browserEnvironment, baseUrl) => {
  return new Promise((resolve, reject) => {
    let skwasmUrl = joinPathSegments(baseUrl, "wimp.js");
    if (deps.flutterTT.policy) {
      skwasmUrl = deps.flutterTT.policy.createScriptURL(skwasmUrl);
    }
    const wasmInstantiator = createWasmInstantiator(joinPathSegments(baseUrl, "wimp.wasm"));
    ...
    script.addEventListener("load", async () => {
      try {
        const skwasmInstance = await wimp({
