...
#ifndef FLUTTER_LIB_WEB_UI_SKWASM_IMPELLER_IMPELLER_STRATEGY_H_
#define FLUTTER_LIB_WEB_UI_SKWASM_IMPELLER_IMPELLER_STRATEGY_H_
...
class Paint : public flutter::DlPaint {
 public:
  ...
  void setStyle(Style style) { setDrawStyle(style); }

  Style getStyle() const { return getDrawStyle(); }

  Color getColor() const { return flutter::DlPaint::getColor().argb(); }
...
class Picture : public flutter::DisplayList {
 public:
  Rect cullRect() { return bounds(); }

  int approximateBytesUsed() { return bytes(); }
};
...
class Canvas {
 public:
  ...
  void restore() { _builder.Restore(); }

  void restoreToCount(int count) { _builder.RestoreToCount(count); }

  int getSaveCount() { return _builder.GetSaveCount(); }

  void translate(Scalar dx, Scalar dy) { _builder.Translate(dx, dy); }

  void scale(Scalar sx, Scalar sy) { _builder.Scale(sx, sy); }

  void rotate(Scalar degrees) { _builder.Rotate(degrees); }

  void skew(Scalar sx, Scalar sy) { _builder.Skew(sx, sy); }

  void concat(const Matrix44& matrix44) { _builder.Transform(matrix44); }
  ...
  void drawPaint(const Paint& paint) { _builder.DrawPaint(paint); }
  ...
  void drawArc(const Rect& rect,
               ...
               bool useCenter,
               const Paint& paint) {
    _builder.DrawArc(rect, startAngleDegrees, sweepAngleDegrees, useCenter,
                     paint);
  }
  ...
  void drawShadow(Path* path,
    ...
    _builder.DrawShadow(*path, flutter::DlColor(color), elevation,
  ...
  void drawImageRect(Image* image,
                     const Rect& sourceRect,
                     const Rect& destRect,
                     SamplingOptions qualtiy,
                     Paint* paint,
                     int) {
    // TODO: draw images
  }

  void drawImageNine(Image* image,
                     const IRect& centerRect,
  ...
  Matrix44 getLocalToDevice() { return _builder.GetTransformFullPerspective(); }

  Rect getLocalClipBounds() { return _builder.GetLocalClipBounds(); }
  ...
  sk_sp<Picture> build() {
    return sk_ref_sp<Picture>(
        static_cast<Picture*>(_builder.Build().release()));
  }
...
static inline void drawShadowOnCanvas(Canvas* canvas,
                                      ...
                                      Color color,
                                      bool transparentOccluder) {
  canvas->drawShadow(path, elevation, devicePixelRatio, color,
                     transparentOccluder);
}
...
class PictureRecorder : public SkRefCnt {
 public:
  ...
  sk_sp<Picture> finishRecordingAsPicture() { return _canvas->build(); }
...
inline sk_sp<SkImage> RasterFromData(const SkImageInfo& info,
                                     sk_sp<SkData> data,
                                     size_t rowBytes) {
  return nullptr;
}
...
inline sk_sp<SkImage> DeferredFromTextureGenerator(
    std::unique_ptr<GrTextureGenerator>&& generator) {
  return nullptr;
}
}  // namespace Images
...
class GraphicsSurface : public SkRefCnt {
 public:
  GraphicsSurface(std::shared_ptr<impeller::ContextGLES> context,
...
class GraphicsContext : public SkRefCnt {
 public:
  GraphicsContext(std::shared_ptr<impeller::ContextGLES> context,
                  std::shared_ptr<impeller::Renderer> renderer,
                  std::shared_ptr<ReactorWorker> worker);

  sk_sp<GraphicsSurface> createSurface(int width, int height);
...
#endif  // FLUTTER_LIB_WEB_UI_SKWASM_IMPELLER_IMPELLER_STRATEGY_H_
