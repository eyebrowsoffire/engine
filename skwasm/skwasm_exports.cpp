...
SurfaceWrapper *createSurfaceFromCanvas(const char *canvasID, int width, int height) {
    ...
    sk_sp<GrDirectContext> grContext = GrDirectContext::MakeGL(GrGLMakeNativeInterface());
...
extern "C"
EMSCRIPTEN_KEEPALIVE
void destroySurface(SurfaceWrapper *wrapper) {
    delete wrapper;
}
...
extern "C"
EMSCRIPTEN_KEEPALIVE
CanvasWrapper *surface_getCanvas(SurfaceWrapper *wrapper) {
    printf("C SURFACE: %p\n", wrapper->surface.get());
    printf("C CANVAS: %p\n", wrapper->surface->getCanvas());
    makeCurrent(wrapper->context);
    return new CanvasWrapper{ wrapper->context, wrapper->surface.get(), wrapper->surface->getCanvas() };
}

extern "C"
EMSCRIPTEN_KEEPALIVE
void canvas_release(CanvasWrapper *wrapper) {
    delete wrapper;
}

extern "C"
EMSCRIPTEN_KEEPALIVE
void canvas_drawCircle(CanvasWrapper *wrapper, float x, float y, float radius) {
    makeCurrent(wrapper->context);

    printf("DRAWING CIRCLE: canvas: %p x: %f y: %f radius: %f\n", wrapper->canvas, x, y, radius);
